...
The Slow Query Log captures SQL statements that take longer than `long_query_time` (default 10 seconds) to execute. JD Cloud provides the `describe-slow-logs` API to query slow query summaries programmatically.
...
|-----|------------|-------------|
| `describeSlowLogs` | `DescribeSlowLogsRequest` | Query slow query summaries by time range (MySQL only) |
...
| `rowsExaminedSum` | Total rows scanned | Identify missing indexes (full scans) |
...
3. **Analyze patterns:**
   ...
   - High `rowsExaminedSum` / low `rowsSentSum` = Missing index (full scan)
   - High `lockTimeSum` = Lock contention issue
4. **Optimize:** Add indexes, rewrite queries, or tune `long_query_time` threshold
