...
```bash
# CLI verification
jdc --output json domainservice describe-domains --page-number 1 --page-size 1
...
client = DomainserviceClient(c)
p = DescribeDomainsParameters(); p.setPageNumber(1); p.setPageSize(1)
print(client.send(DescribeDomainsRequest(parameters=p)).result)
"
```
...
```bash
# In CI pipeline, set env vars and use jdc CLI
