...
def test_simple_chain_vpc_subnet_vm():
    ...
    resources = [
        (
            "vm",
            {"instanceId": "i-1", "subnetId": "subnet-1", "instanceName": "vm1"},
            MappingSpec("vm", "jdcloud_instance", parent_ref="subnetId", rules=[]),
            "vm1",
        ),
        (
            "subnet",
            {"subnetId": "subnet-1", "vpcId": "vpc-1", "subnetName": "sub1"},
            MappingSpec("subnet", "jdcloud_subnet", parent_ref="vpcId", rules=[]),
            "sub1",
        ),
        (
            "vpc",
            {"vpcId": "vpc-1", "vpcName": "v1"},
            MappingSpec("vpc", "jdcloud_vpc", rules=[]),
            "v1",
        ),
    ]
...
def test_disconnected_graph_still_ordered():
    ...
    resources = [
        ...
        (
            "iam",
            {"subUserName": "ops1"},
            MappingSpec("iam", "jdcloud_iam_sub_user", rules=[]),
            "ops1",
        ),
    ]
    ordered = infer_dependencies(resources)
...
def test_no_parent_ref_means_root():
    ...
    resources = [
        (
            "eip",
            {"elasticIpId": "fip-1", "name": "eip1"},
            MappingSpec("eip", "jdcloud_eip", rules=[]),
            "eip1",
        ),
    ]
    ordered = infer_dependencies(resources)
...
def test_missing_parent_id_means_orphan():
    ...
    resources = [
        (
            "vm",
            {"instanceId": "i-1", "instanceName": "vm1"},  # no subnetId
            MappingSpec("vm", "jdcloud_instance", parent_ref="subnetId", rules=[]),
            "vm1",
        ),
        (
...
def test_circular_dependency_documented():
    ...
    cycle_resources = [
        (
            "custom_a",
            {"aId": "a-1", "bId": "b-1"},
            MappingSpec("custom_a", "x_a", parent_ref="bId", rules=[]),
            "a",
        ),
        (
            "custom_b",
            {"bId": "b-1", "aId": "a-1"},
            MappingSpec("custom_b", "x_b", parent_ref="aId", rules=[]),
            "b",
        ),
    ]
...
def test_jdcloud_specific_chain_clb_eip(load_fixture):
    ...
    resources = [
        (
            "eip",
            {"elasticIpId": "fip-1", "name": "e1"},
            MappingSpec("eip", "jdcloud_eip", rules=[]),
            "e1",
        ),
        (
            "clb",
            {"loadBalancerId": "lb-1", "loadBalancerName": "lb1"},
            MappingSpec("clb", "jdcloud_lb", rules=[]),
            "lb1",
        ),
    ]
    ordered = infer_dependencies(resources)
