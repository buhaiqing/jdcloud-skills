# Failure Patterns — jdcloud-topo-discovery
...
> **Note**: `jdcloud-topo-discovery` is a meta skill — it renders topology
> graphs over `jdcloud-aiops-cruise` output rather than performing direct
> resource CRUD. Command examples below reflect graph rendering workflows
> and downstream delegation calls.
...
- **Command**: `jdc topo describe-graph --graph-id <invalid>`
- **Error**: `InvalidParameter: InvalidResourceId`
- **Fix**: Resource ID must be in valid format (graph UUID)
- **Count**: 1
- **Reusable**: true
...
- **Category**: cli_parameter
- **Skill**: jdcloud-topo-discovery
- **Command**: `jdc topo describe-graphs` (missing --region-id)
...
- **Category**: cli_parameter
- **Skill**: jdcloud-topo-discovery
- **Command**: `jdc topo describe-graphs --output json` (wrong position)
...
- **Skill**: jdcloud-topo-discovery
- **Command**: `jdc topo delete-graph --graph-id <id>` (when graph is in use)
- **Error**: `ResourceInUse`
...
- **Skill**: jdcloud-topo-discovery
- **Command**: `jdc topo describe-graphs --page-size 100`
...
| Version | Date | Change |
|---------|------|--------|
| 1.0.0 | 2026-06-19 | Initial failure patterns for `jdcloud-topo-discovery` |
