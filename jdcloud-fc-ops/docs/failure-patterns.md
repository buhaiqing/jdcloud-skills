# Failure Patterns — jdcloud-fc-ops
...
> **Note**: `jdcloud-fc-ops` is SDK-only — Function Compute is not exposed via the `jdc` CLI.
...
- **Command**: `client.describe_function(function_name='<invalid>')`
- **Error**: `InvalidParameter: InvalidResourceId`
- **Fix**: Resource ID must be in valid format (function name must match allowed pattern)
- **Count**: 1
- **Reusable**: true
...
- **Category**: cli_parameter
- **Skill**: jdcloud-fc-ops
- **Command**: `client.list_functions()` (missing region_id)
...
- **Category**: cli_parameter
- **Skill**: jdcloud-fc-ops
- **Command**: `client.list_functions(output='json')` (not supported in SDK)
...
- **Skill**: jdcloud-fc-ops
- **Command**: `client.delete_function(function_name='<name>')` (when function has active triggers)
- **Error**: `ResourceInUse`
- **Fix**: Stop/detach dependent resources first before deleting (remove triggers first)
- **Count**: 1
- **Reusable**: true
...
- **Category**: skill_generation
- **Skill**: jdcloud-fc-ops
- **Command**: `client.create_function(function_name='<name>')`
...
- **Skill**: jdcloud-fc-ops
- **Command**: `client.list_functions(limit=100)`
- **Warning**: Response too large, consumes excessive tokens
- **Fix**: Use smaller limit or add filters to narrow results
- **Count**: 1
- **Reusable**: true
...
| Version | Date | Change |
|---------|------|--------|
| 1.0.0 | 2026-06-19 | Initial failure patterns for `jdcloud-fc-ops` |
