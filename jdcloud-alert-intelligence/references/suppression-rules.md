...
**维护窗口内的告警一律"过滤"，不进 P0/P1 详单**（但仍计入"告警总量"用于统计）。

| 状态 | 报告中位置 |
|---|---|
| 维护窗内告警 | "已被抑制" 列表，**不出** P0/P1 详单 |
...
**核心规则：** 当前告警簇若在过去 7 天同时段（±30min）出现 ≥ 3 次，视为"已知周期性"，降档为 P3。
...
**v0.1 简化：** 命中即降 P3，**不再细分**"低频周期"和"高频周期"。
...
flowchart TD
    A[告警簇进入抑制层] --> B{维护窗口命中?}
    B -- 是 --> Z1[过滤<br/>不入P0/P1详单<br/>计入"已被抑制"]
    B -- 否 --> C{命中历史7d同时段≥3次?}
    C -- 是 --> D[降档→P3]
    C -- 否 --> E{命中已知误报5类?}
    E -- 是 --> F[降档→P3<br/>跨级豁免]
