...
**重要原则：先分级 → 后抑制**。抑制步骤（playbook-suppress.md）会**降级**已分好的标签。
...
- 抑制判定依赖"已分好的级别"（如 P0 不允许被维护窗直接过滤，必须降级为 P1 并标注）
- 反过来：若先抑制，P3 周期性告警无法进入"分级"流水线，会被误过滤

**抑制阶段的降级方向**（详见 [playbook-suppress.md](./playbook-suppress.md)）：

| 原级别 | 命中维护窗 | 命中周期性 | 命中已知误报 |
|---|---|---|---|
| P0 | 降 P1 | 降 P1 | 降 P3（跨级豁免 + 报告标注）|
| P1 | 降 P2 | 降 P2 | 降 P3（跨级豁免 + 报告标注）|
| P2 | 降 P3 | 降 P3 | 降 P3（跨级豁免 + 报告标注）|
| P3 | 不变 | 不变 | 不变 |
| P3 | 过滤 | 过滤 | 过滤 |
