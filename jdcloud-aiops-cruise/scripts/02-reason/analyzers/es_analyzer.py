"""
jdcloud-aiops-cruise / analyzers / es_analyzer.py
=================================================
    ...
    def analyze(self) -> list:
        ...
        for es in self.resources:
            ...
            if kibana:
                ...
                self._add_finding("info",
                    f"Kibana已启用: {kibana_url[:60]}..", "", name)
            ...
            for mname in ["cpu_util", "memory.usage"]:
                ...
                if pts:
                    ...
                    if mname == "cpu_util" and avg > 70:
                        self._add_finding("warning",
                            f"CPU平均{avg:.1f}%", "检查ES查询负载", name)
                    elif mname == "memory.usage" and avg > 80:
                        self._add_finding("warning",
                            f"内存平均{avg:.1f}%", "检查JVM堆内存使用", name)
