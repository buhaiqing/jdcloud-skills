"""
jdcloud-aiops-cruise / analyzers / mongodb_analyzer.py
=====================================================
        ...
        for r in self.resources:
            ...
            if status and status != "running":
                self._add_finding(
                    "critical",
                    f"实例状态异常: {status}",
                    "检查最近备份/恢复/变更任务；若长时间非 running，携带 requestId 联系京东云支持",
                    name,
                    rid,
                    instance_type=instance_type,
                    ops_skill="jdcloud-mongodb-ops",
                )
            ...
            env = tags.get("环境", "")
            if env:
                self._add_finding("info", f"环境: {env}", "", name, rid, instance_type=instance_type)
