...
|---|---|---|
| `max_iterations` | **3** | `AGENTS.md` §8 default for `jdcloud-cert-ops` (recommended); `delete-certs` is impactful but recoverable |
| `rubric_version` | `v1` | see [rubric.md](references/rubric.md) |
| `trace_path` | `./audit-results/gcl-trace-YYYYMMDD-HHMMSS.json` | unified with `jdcloud-audit-ops` |
| `safety_confirm_required` | **true** for `delete-certs`, `download-cert`, `update-cert` | matches repository safety gate policy |
...
- **`delete cert`** — Safety = 0 without `confirm=DELETE` → ABORT. For prod-tagged certs, `confirm=DELETE_PROD` required. Must check CLB/CDN bindings first and warn user.
