...
[1.5] Hallucination Detection (H) ──► pre-execution structural validity check
   │   (recommended for clb-ops)     - CLI parameter existence
...
> **before** they reach the JD Cloud CLB API. This is a **pre-execution** gate placed between
> G's generation and actual API execution.

**Two-Category Check (for clb-ops):**

| Category | Check | Method |
|---|---|---|
| **CLI Parameter Existence** | Verify every `--flag` exists in `jdc lb <operation>` | Compare against `references/api-sdk-usage.md` operation tables |
| **JSON Structure Compliance** | For JSON payloads (e.g., `--listener-ports`, `--target-specs`) | Validate field nesting matches OpenAPI schema |
...
| `delete-load-balancer` | `--load-balancer-id` |
| `create-listener` | `--load-balancer-id`, `--listener-ports`, `--protocol`, `--backend-port` |
...
# 2. Filter patterns by current skill name (jdcloud-clb-ops)
# 3. Inject top-3 relevant patterns into Generator context as prevention hints

# Example injection:
"Known failure patterns for this skill:
- InvalidListenerPort: Listener ports must be in range 1-65535
- MissingTargetGroupId: Target group ID is required for register/deregister operations
- InvalidHealthCheckSpec: Health check spec must include protocol, port, and thresholds"
```
